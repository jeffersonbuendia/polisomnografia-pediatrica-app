...
st.set_page_config(
    page_title="PSG Pediátrica - Reporte hospitalario validado",
    page_icon="💤",
    layout="wide"
)
...
st.markdown(
    '<div class="subtitle">Versión hospitalaria premium validada: reporte clínico estructurado en PDF y Word editable.</div>',
    unsafe_allow_html=True
)
...
def interpretar_odi(odi: float, tipo_odi: str, edad_anios: float) -> str:
    """
    Criterios orientativos basados en recomendaciones pediátricas:
    ODI4 >4/h o ODI3 >7/h sugieren anormalidad en niños mayores de 2 años.
    En menores de 2 años se requiere mayor cautela interpretativa.
    """
    if tipo_odi == "ODI4":
        if odi > 4:
            resultado = "ODI4 elevado para edad pediátrica, compatible con carga anormal de desaturaciones"
        else:
            resultado = "ODI4 dentro de rango no claramente anormal"
    else:
        if odi > 7:
            resultado = "ODI3 elevado para edad pediátrica, compatible con carga anormal de desaturaciones"
        else:
            resultado = "ODI3 dentro de rango no claramente anormal"

    if edad_anios < 2:
        resultado += "; interpretar con cautela por edad menor de 2 años"
    return resultado
...
def interpretar_arousals(indice_arousal: float) -> str:
    """
    Interpretación orientativa: no existe un punto de corte pediátrico universal único.
    """
    if indice_arousal < 10:
        return "índice de arousal bajo o no claramente aumentado, interpretación orientativa"
    if indice_arousal < 20:
        return "índice de arousal moderadamente aumentado, interpretación orientativa"
    return "índice de arousal marcadamente aumentado, interpretación orientativa"
...
def advertencia_edad(edad_anios: float) -> str:
    if edad_anios < 2:
        return (
            "Advertencia clínica: en menores de 2 años la interpretación de eventos respiratorios "
            "y desaturaciones requiere especial cautela; se recomienda integrar el resultado con "
            "la edad corregida, comorbilidades, patrón de respiración periódica y criterio del laboratorio."
        )
    return ""
...
def make_impression(data: dict) -> str:
    edad_txt = f"Edad registrada: {data['edad_anios']:.2f} años. " if data["edad_anios"] > 0 else ""
    adv = f" {data['advertencia_edad']}" if data["advertencia_edad"] else ""
    return (
        f"{edad_txt}La polisomnografía pediátrica muestra un tiempo total de sueño de "
        ...
        f"La saturación basal fue de {data['spo2_basal']:.1f}%, con nadir de "
        f"{data['spo2_min']:.1f}% y tiempo con SpO₂ <90% de "
        f"{data['tiempo_menor_90']:.1f}% del tiempo total de sueño, lo cual sugiere "
        f"{data['interp_oxigenacion']}. El {data['tipo_odi']} fue de "
        f"{data['odi']:.1f} eventos/hora; {data['interp_odi']}. "
        f"El índice de arousal fue de {data['indice_arousal']:.1f} eventos/hora, "
        f"con interpretación como {data['interp_arousal']}. "
        ...
        f"oxigenación descrita como {data['interp_oxigenacion']}.{adv}"
    )
...
def build_pdf(data: dict) -> bytes:
    ...
    title_style = ParagraphStyle("TitleCustom", parent=styles["Title"], fontName="Helvetica-Bold", fontSize=15,
                                 leading=18, alignment=TA_CENTER, textColor=colors.HexColor("#111827"), spaceAfter=6)
    institution_style = ParagraphStyle("InstitutionCustom", parent=styles["Normal"], fontName="Helvetica-Bold",
                                       fontSize=10, leading=12, alignment=TA_CENTER, textColor=colors.HexColor("#1F2937"), spaceAfter=2)
    date_style = ParagraphStyle("DateCustom", parent=styles["Normal"], fontName="Helvetica", fontSize=8.5,
                                leading=10, alignment=TA_CENTER, textColor=colors.HexColor("#4B5563"), spaceAfter=8)
    h_style = ParagraphStyle("HeadingCustom", parent=styles["Heading2"], fontName="Helvetica-Bold", fontSize=10.5,
                             leading=13, textColor=colors.HexColor("#111827"), spaceBefore=7, spaceAfter=4)
    normal_style = ParagraphStyle("NormalCustom", parent=styles["Normal"], fontName="Helvetica", fontSize=8.8,
                                  leading=11.5, alignment=TA_JUSTIFY, textColor=colors.HexColor("#111827"))
    note_style = ParagraphStyle("NoteCustom", parent=styles["Normal"], fontName="Helvetica", fontSize=7.8,
                                leading=9.6, alignment=TA_LEFT, textColor=colors.HexColor("#4B5563"))
    ...
    table([
        ["Parámetro", "Resultado"],
        ["Edad", f'{data["edad_anios"]:.2f} años'],
    ...
    table([["Estadio", "% del TST"], ["N1", f'{data["n1"]:.1f}%'], ["N2", f'{data["n2"]:.1f}%'],
           ["N3", f'{data["n3"]:.1f}%'], ["REM", f'{data["rem"]:.1f}%']], widths=[9 * cm, 7.3 * cm])
    p(f'Interpretación: el estudio muestra {data["interp_eficiencia"]}.')
    ...
    table([
        ...
        [data["tipo_odi"], f'{data["odi"]:.1f} eventos/hora'],
    ], widths=[9 * cm, 7.3 * cm])
    p(f'Interpretación: se documenta {data["interp_oxigenacion"]}. {data["interp_odi"]}.')

    section("6. Eventos cardiovasculares")
    table([["Parámetro", "Resultado"], ["Frecuencia cardíaca promedio", f'{data["fc_promedio"]:.1f} lpm'],
           ["Frecuencia cardíaca mínima", f'{data["fc_min"]:.1f} lpm'],
           ["Frecuencia cardíaca máxima", f'{data["fc_max"]:.1f} lpm'], ["Arritmias", data["arritmias"]]],
          widths=[9 * cm, 7.3 * cm])

    section("7. Movimientos periódicos de extremidades")
    table([["Parámetro", "Resultado"], ["PLMI", f'{data["plmi"]:.1f} eventos/hora']], widths=[9 * cm, 7.3 * cm])
    p(f'Interpretación: {data["interp_plmi"]}.')

    section("8. Microdespertares")
    table([["Parámetro", "Resultado"], ["Índice de arousal", f'{data["indice_arousal"]:.1f} eventos/hora']], widths=[9 * cm, 7.3 * cm])
    p(f'Interpretación: {data["interp_arousal"]}. No existe un punto de corte pediátrico universal único para severidad basada solo en arousals.')

    section("9. Análisis posicional")
    table([["Posición", "IAH"], ["Supino", f'{data["iah_supino"]:.1f} eventos/hora'],
           ["Lateral", f'{data["iah_lateral"]:.1f} eventos/hora']], widths=[9 * cm, 7.3 * cm])
    ...
    p(
        ...
        f'El {data["tipo_odi"]} fue de <b>{data["odi"]:.1f} eventos/hora</b>: {data["interp_odi"]}.'
    )

    section("12. Impresión clínica integrada")
    p(safe_text(data["impresion"]))

    if data["advertencia_edad"]:
        section("13. Advertencia por edad")
        p(safe_text(data["advertencia_edad"]))
    ...
    if data.get("mostrar_nota"):
        ...
        story.append(Paragraph(
            ...
            "Para ODI, se interpreta ODI4 >4/h u ODI3 >7/h como anormal en mayores de 2 años. "
    ...
    if data.get("medico_lector", "").strip() or data.get("registro_medico", "").strip():
        ...
        table([["Firma / responsable", ""], ["Médico lector", data.get("medico_lector", "")],
               ["Registro profesional", data.get("registro_medico", "")]], widths=[6 * cm, 10.3 * cm])
...
def build_docx(data: dict) -> bytes:
    ...
    add_docx_table(doc, [["Estadio", "% del TST"], ["N1", f'{data["n1"]:.1f}%'], ["N2", f'{data["n2"]:.1f}%'],
                         ["N3", f'{data["n3"]:.1f}%'], ["REM", f'{data["rem"]:.1f}%']])
    add_docx_paragraph(doc, f'Interpretación: el estudio muestra {data["interp_eficiencia"]}.')

    add_docx_section(doc, "4. Eventos respiratorios")
    add_docx_table(doc, [["Parámetro", "Resultado"], ["IAH total", f'{data["iah"]:.1f} eventos/hora'],
                         ["IAH obstructivo", f'{data["iah_obstructivo"]:.1f} eventos/hora'],
                         ["IAH central", f'{data["iah_central"]:.1f} eventos/hora'],
                         ["Índice de hipopneas", f'{data["indice_hipopneas"]:.1f} eventos/hora'],
                         ["IAH REM", f'{data["iah_rem"]:.1f} eventos/hora'],
    ...
    add_docx_table(doc, [["Parámetro", "Resultado"], ["SpO₂ basal", f'{data["spo2_basal"]:.1f}%'],
                         ["SpO₂ mínima", f'{data["spo2_min"]:.1f}%'],
                         ["Tiempo con SpO₂ <90%", f'{data["tiempo_menor_90"]:.1f}% del TST'],
                         [data["tipo_odi"], f'{data["odi"]:.1f} eventos/hora']])
    add_docx_paragraph(doc, f'Interpretación: se documenta {data["interp_oxigenacion"]}. {data["interp_odi"]}.')

    add_docx_section(doc, "6. Eventos cardiovasculares")
    add_docx_table(doc, [["Parámetro", "Resultado"], ["Frecuencia cardíaca promedio", f'{data["fc_promedio"]:.1f} lpm'],
                         ["Frecuencia cardíaca mínima", f'{data["fc_min"]:.1f} lpm'],
                         ["Frecuencia cardíaca máxima", f'{data["fc_max"]:.1f} lpm'], ["Arritmias", data["arritmias"]]])

    add_docx_section(doc, "7. Movimientos periódicos de extremidades")
    add_docx_table(doc, [["Parámetro", "Resultado"], ["PLMI", f'{data["plmi"]:.1f} eventos/hora']])
    add_docx_paragraph(doc, f'Interpretación: {data["interp_plmi"]}.')

    add_docx_section(doc, "8. Microdespertares")
    add_docx_table(doc, [["Parámetro", "Resultado"], ["Índice de arousal", f'{data["indice_arousal"]:.1f} eventos/hora']])
    add_docx_paragraph(doc, f'Interpretación: {data["interp_arousal"]}. No existe un punto de corte pediátrico universal único para severidad basada solo en arousals.')

    add_docx_section(doc, "9. Análisis posicional")
    add_docx_table(doc, [["Posición", "IAH"], ["Supino", f'{data["iah_supino"]:.1f} eventos/hora'],
    ...
    add_docx_paragraph(doc, f'Estudio compatible con {data["diagnostico_iah"]}, con IAH total de {data["iah"]:.1f} eventos/hora, IAH obstructivo de {data["iah_obstructivo"]:.1f} eventos/hora, saturación mínima de {data["spo2_min"]:.1f}% y {data["interp_oxigenacion"]}. El {data["tipo_odi"]} fue de {data["odi"]:.1f} eventos/hora: {data["interp_odi"]}.')

    add_docx_section(doc, "12. Impresión clínica integrada")
    add_docx_paragraph(doc, data["impresion"])

    if data["advertencia_edad"]:
        add_docx_section(doc, "13. Advertencia por edad")
        add_docx_paragraph(doc, data["advertencia_edad"])

    if data.get("mostrar_nota"):
        add_docx_section(doc, "Nota técnica")
        add_docx_paragraph(doc, "La clasificación automática usa umbrales pediátricos habituales del IAH (normal <1; leve 1 a <5; moderada 5 a <10; severa >=10 eventos/hora). Para ODI, se interpreta ODI4 >4/h u ODI3 >7/h como anormal en mayores de 2 años. La interpretación final debe integrarse con edad, síntomas, comorbilidades, examen físico y criterios vigentes del laboratorio de sueño.")

    if data.get("medico_lector", "").strip() or data.get("registro_medico", "").strip():
        doc.add_paragraph("")
        add_docx_table(doc, [["Firma / responsable", ""], ["Médico lector", data.get("medico_lector", "")],
...
indicacion = st.text_area("Indicación", value="Evaluación de trastorno respiratorio del sueño en paciente pediátrico.", height=80)

st.header("2. Condiciones del estudio")
col1, col2, col3 = st.columns(3)
with col1:
    fecha_estudio = st.date_input("Fecha del estudio", value=date.today())
    edad_anios = st.number_input("Edad del paciente (años)", min_value=0.0, max_value=18.0, value=6.0, step=0.1)
    tipo_estudio = st.selectbox("Tipo de estudio", ["Polisomnografía nocturna completa nivel I", "Polisomnografía hospitalaria", "Polisomnografía ambulatoria", "Otro"])
...
if edad_anios < 2:
    st.warning("Menor de 2 años: la interpretación de desaturaciones, eventos centrales y patrón respiratorio requiere cautela clínica adicional.")
...
with col4:
    tipo_odi = st.selectbox("Tipo de ODI", ["ODI3", "ODI4"], index=1)
with col5:
    odi = st.number_input("ODI (eventos/hora)", min_value=0.0, value=4.0, step=0.1)
...
artefactos = st.text_area("Canales perdidos o artefactos relevantes", value="Sin artefactos significativos que limiten la interpretación.", height=80)
...
interp_odi = interpretar_odi(odi, tipo_odi, edad_anios)
...
adv_edad = advertencia_edad(edad_anios)

data = {
    "institucion": institucion, "medico_lector": medico_lector, "registro_medico": registro_medico,
    "mostrar_nota": mostrar_nota, "indicacion": indicacion, "fecha_estudio": fecha_estudio,
    "edad_anios": edad_anios, "tipo_estudio": tipo_estudio, "duracion_registro": duracion_registro,
    "tiempo_cama": tiempo_cama, "tiempo_sueno": tiempo_sueno, "eficiencia": eficiencia,
    "latencia_sueno": latencia_sueno, "latencia_rem": latencia_rem,
    "posicion_predominante": posicion_predominante, "n1": n1, "n2": n2, "n3": n3, "rem": rem,
    "iah": iah, "iah_obstructivo": iah_obstructivo, "iah_central": iah_central,
    "indice_hipopneas": indice_hipopneas, "iah_rem": iah_rem, "iah_nrem": iah_nrem,
    "spo2_basal": spo2_basal, "spo2_min": spo2_min, "tiempo_menor_90": tiempo_menor_90,
    "tipo_odi": tipo_odi, "odi": odi, "fc_promedio": fc_promedio, "fc_min": fc_min, "fc_max": fc_max,
    "arritmias": arritmias, "plmi": plmi, "indice_arousal": indice_arousal,
    "iah_supino": iah_supino, "iah_lateral": iah_lateral, "calidad": calidad, "artefactos": artefactos,
    "diagnostico_iah": diagnostico_iah, "grado_corto": grado_corto,
    "interp_oxigenacion": interp_oxigenacion, "interp_odi": interp_odi,
    "interp_eficiencia": interp_eficiencia, "interp_arousal": interp_arousal,
    "interp_plmi": interp_plmi, "frase_rem": frase_rem, "frase_posicion": frase_posicion,
    "advertencia_edad": adv_edad,
}
...
m3.metric(tipo_odi, f"{odi:.1f}/h")
m4.metric("Arousal", f"{indice_arousal:.1f}/h")
m5.metric("PLMI", f"{plmi:.1f}/h")

st.markdown(f"""<div class="report-box"><b>Impresión clínica integrada:</b><br><br>{data["impresion"]}</div>""", unsafe_allow_html=True)
...
col1, col2 = st.columns(2)
with col1:
    st.download_button("📄 Descargar reporte en PDF", data=pdf_bytes, file_name="reporte_polisomnografia_pediatrica.pdf", mime="application/pdf", use_container_width=True)
with col2:
    st.download_button("📝 Descargar reporte en Word (.docx)", data=docx_bytes, file_name="reporte_polisomnografia_pediatrica.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", use_container_width=True)
